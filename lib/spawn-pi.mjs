...
export class PiRpc {
    ...
    _onLine(line) {
        ...
        // Use agent_settled (fully settled, no retry/compaction/queued continuation
        // pending) instead of agent_end (may still retry / compact). See
        // D:\dev-cache\npm-global\node_modules\@earendil-works\pi-coding-agent\docs\rpc.md
        // for the event taxonomy.
        if (msg && msg.type === "agent_settled") {
            const resolvers = this._assistantDoneResolvers;
            this._assistantDoneResolvers = [];
            for (const r of resolvers) r({ text: this._assistantText, toolCount: this._toolExecCount, settled: true });
        }
